...
function Tracer:start(upstream_name, correlation)
    ...
    -- ============源码修改===============
    return tracingContext.trace_id
    -- ============源码修改===============
end
-- ============源码修改===============
function Tracer:set_operation_name(ability_code)
    local spans=ngx.ctx.tracingContext.internal.active_spans
    for _,span in ipairs(spans) do
        span.operation_name = ability_code
    end
end
-- ============源码修改===============
...
-- ============源码修改===============
-- 记录服务调用信息
function Tracer:service_start(service_info)
    local tracingContext = ngx.ctx.tracingContext
    if tracingContext == nil then
        return
    end
    local entrySpan = ngx.ctx.entrySpan
    local service_code = service_info.CODE
    local provider_name = service_info.APPLYERID
    local serviceSpan = TC.createLocalSpan(tracingContext,service_code,entrySpan,provider_name)
    Span.start(serviceSpan, ngx.now() * 1000)
    Span.setComponentId(serviceSpan, NGINX_COMPONENT_ID)
    Span.setLayer(serviceSpan, Layer.HTTP)
    core.log.info("service_info:",core.json.delay_encode(service_info))
    Span.tag(serviceSpan, 'http.method', service_info.HTTPMETHOD)
    Span.tag(serviceSpan,'format',service_info.FORMAT)
    Span.tag(serviceSpan,'encoding',service_info.ENCODING)
    ngx.ctx.serviceSpan = serviceSpan
    core.log.info("serviceSpan:",core.json.delay_encode(serviceSpan,true))
end

function Tracer:service_finish(status)
    local serviceSpan = ngx.ctx.serviceSpan
    if serviceSpan == nil then
        return
    end
    Span.finish(serviceSpan, ngx.now() * 1000)
    if status then
        Span.tag(serviceSpan, 'http.status', status)
    end
    Span.tag(serviceSpan,"duration",serviceSpan.end_time - serviceSpan.start_time)
    ngx.ctx.serviceSpan = nil
end
-- ============源码修改===============
